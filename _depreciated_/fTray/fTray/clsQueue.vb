...
#Region "Declares"
    ...
    Public Event NewFrame(ByVal CurrentProgress As Int16, ByVal FrameTime As TimeSpan, ByVal dtFrameEnd As DateTime)
    Public Event ProjectEnd(ByVal Slot As Entry, ByVal FrameTime As TimeSpan, ByVal dtFrameBegin As DateTime, ByVal Percentage As Short)
...
#Region "Properties"
    ...
    Public ReadOnly Property PointsInUpdate() As Double
        ...
        End Get
    End Property
    Public ReadOnly Property EUE_Count() As Int16
        Get
            Return _EUE
        End Get
    End Property
    Public ReadOnly Property Progress() As Int16
        Get
            Return _Progress
        End Get
    End Property
    ...
    Public Function CheckCoreExit() As Boolean
        Try
            ...
            If Not cStatus.IsEmpty Then
                ...
                For Each nfString As String In MySettings.MySettings.NonFatal
...
#End Region
    Private Sub fwError(ByVal Sender As Object, ByVal e As System.IO.ErrorEventArgs)
        Try
            LogWindow.WriteLog("clsQueue, FWerror : " & e.GetException.Message)
            ...
            End If
        Catch ex As Exception
            LogWindow.WriteError("clsQueue, fwError", Err)
        End Try
    End Sub
    Private _bReportErrors As Boolean = False, iFailureCount As Int16 = 0
    Private Sub QueueEvent(ByVal Sender As Object, ByVal e As System.IO.FileSystemEventArgs)
        ...
        Try
            ...
            Dim bActiveSlot As Int16 = _Current
            Dim biCurrent As Int16 = ActiveSlot.Progress
            If ReadQueue() Then
            ...
            Else
                LogWindow.WriteLog("QueueEvent could not be procesed due to previous errors ( ReadQueue? )")
            End If
            bIsBussy = False
            LogWindow.WriteLog("-Queue event handeld in " & DateTime.Now.Subtract(dtStart).TotalMilliseconds & "ms")
        Catch ex As Exception
            If _bReportErrors Then
                LogWindow.WriteError("clsQueue, QueueEvent", Err)
            Else
                iFailureCount += 1
        ...
        End Try
    End Sub
    Public ReadOnly Property CoreStatus(ByVal Entry) As clsCoreStatus
        Get
            Try
                ...
                If Log.Contains(vbNewLine) Then
                    Lines = Log.Split(vbNewLine)
                ElseIf Log.Contains(vbCrLf) Then
                    Lines = Log.Split(vbCrLf)
                ElseIf Log.Contains(vbLf) Then
                    Lines = Log.Split(vbLf)
                End If
                Dim iStart As Int16 = -1 : Dim iEnd As Int16 = -1 : Dim iStatus As Int16 = -1 : Dim iProject As Int16 = -1
                For xInd As Int16 = Lines.GetUpperBound(0) To (Lines.GetUpperBound(0) - 150) Step -1
...
ReturnNothing:
                ...
                Return rCoreStatus
            Catch ex As Exception
                LogWindow.WriteError("clsCoreStatus, CoreStatus", Err, "CoreStatus object could not be generated for " & ActiveSlot.Project.Project & " from location " & _logfile_)
    ...
    Private Function PPDLastFrame() As Double
        Try
            If _tsFrame.TotalSeconds = 0 Or Not ProjectInfo.Projects.KnownProject(ActiveSlot.Project.Project) Then Return 0
            'get kFactor
            Dim iKfactor As Double = CDbl(ProjectInfo.Projects.Project(ActiveSlot.Project.Project).kFactor.Replace(".", ","))
            Dim iPworth As Double = CDbl(ProjectInfo.Projects.Project(ActiveSlot.Project.Project).Credit.Replace(".", ","))
            If iKfactor > 0 Then
                'check if eta is before preferred
                If _EtaDT < ActiveSlot.Issued.AddDays(ProjectInfo.Projects.Project(ActiveSlot.Project.Project).PreferredDays.Replace(".", ",")) Then
                    ...
                    Dim bMulti As Double = Math.Sqrt((ProjectInfo.Projects.Project(ActiveSlot.Project.Project).PreferredDays.Replace(".", ",") * iKfactor) / uEst.TotalDays)
        ...
        Catch ex As Exception
            LogWindow.WriteError("PPD_LastFrame", Err, _Location)
            Return 0
        End Try
    End Function
    Private Function PPDEffective() As Double
        Try
            ...
            For xInt As Int16 = _Progress To 100
        ...
        End Try
    End Function
    Private Function CurrentProgress() As Int16
        Try
                    ...
                    For xInd As Int16 = Lines.GetUpperBound(0) To 0 Step -1
                        With Lines(xInd)
                            ...
                            ElseIf .Contains("FINISHED A FRAME") Then
                                'Get total frames
                                If ProjectInfo.Projects.KnownProject(ActiveSlot.Project.Project) Then
                                    Dim iTFrames As Integer = CInt(ProjectInfo.Projects.Project(ActiveSlot.Project.Project).Frames)
                    ...
                    For xInd As Integer = Lines.GetUpperBound(0) To 0 Step -1
                        With Lines(xInd).ToUpper
                            If .Contains("Arguments:".ToUpper) Then
                                ...
                                _Arguments = _Arguments.Trim(vbCrLf)
                            ElseIf .Contains("Folding@Home Client Version".ToUpper) Then
                                ...
                                _ClientVersion = _ClientVersion.TrimEnd(vbLf).TrimEnd(vbNewLine).TrimEnd(vbCrLf)
                            ...
                            If .Contains("COMPLETED") And Not .Contains("STEPS") And Not .Contains("UNITS") And Not .Contains("UNSENT") And Not .Contains("AUTOSEND") Then
                                ...
                                dL = DateTime.UtcNow.ToShortDateString
                                ...
                                For xInd2 As Int16 = xInd - 1 To 0 Step -1
                                    With Lines(xInd2).ToUpper
                                        If .Contains("COMPLETED") And Not .Contains("UNITS") And Not .Contains("UNSENT") And Not .Contains("REMAINING") And Not .Contains("AUTOSEND") Then
                                            ...
                                            dB = DateTime.UtcNow.ToShortDateString
...
Skip3:
    ...
    Private Function GetCoreVersion() As String
        Try
                    ...
                    With Lines(xInd).ToUpper
                        ...
                        If .Contains("*------------------------------*") Then
                            ...
                            If Lines(xInd + 2).ToUpper.Contains("VERSION") Then
                                ...
                                strVer = Mid(strVer, 1, strVer.LastIndexOf("(") - 1).ToUpper.Replace("VERSION", "").Trim(" ")
                                _CoreVersion = strVer
                                bCore = True
                            End If
            ...
            End If
        Catch ex As Exception
            LogWindow.WriteError("GetCoreVersion", Err)
            Return ""
        End Try
    End Function
    Private Function GetClientVersion() As String
        Try
            ...
            Next
        Catch ex As Exception
            LogWindow.WriteError("GetClientVersion", Err)
            Return ""
        End Try
    End Function
#End Region
#Region "Core status"
    ...
    Public Class clsCoreStatus
        ...
        Public ReadOnly Property SnippetSize() As Int16
            Get
                Try
...
#Region "Queue.dat"
    ...
    Private _TotalUploadFailures As Int16 = 0
    Private _WUsReady As Int16 = 0
    ...
    Public ReadOnly Property StatsFile() As String
        Get
            Try
                ...
                Return strRet
            Catch ex As Exception
                LogWindow.WriteError("clsQueue, Statsfile", Err, ex.Message)
                Return vbNullString
            End Try
        End Get
    End Property
    Public ReadOnly Property Version() As String
        ...
        End Get
    End Property
    Public ReadOnly Property Current() As Int16
        Get
            Return _Current
        End Get
    End Property
    ...
    Public ReadOnly Property UploadRateUnits() As String
        ...
        End Get
    End Property
    Public ReadOnly Property TotalUploadFailures() As Int16
        Get
            Return _TotalUploadFailures
        End Get
    End Property
    Public ReadOnly Property WUs_Ready() As Int16
        Get
            Return _WUsReady
        End Get
    End Property
    ...
    Public ReadOnly Property ActiveSlot() As Entry
        ...
        End Get
    End Property
    Public ReadOnly Property Slot(ByVal Index As Int16) As Entry
        Get
            Try
                If Not _FullSlots Then Return Nothing
    ...
    Public Class Entry
        ...
        Private iProgress As Int16
        Public Property Progress() As Int16
            Get
                Return iProgress
            End Get
    ...
    Public Function ReadQueue() As Boolean
        Try
            ...
            For xInd As Int16 = 0 To 9
                ...
                With Entries(xInd)
                    ...
                    End Try
                    Try
                        .SmpCores = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'TimeData
                    ReDim fBuff(0 To 31)
                    Try
                    ...
                    Try
                        ...
                        Else
                            .TimeData.BeginTime = DateTime.UtcNow
                            .TimeData.BeginTime = #1/1/2000#.AddSeconds(dStart)
                            .TimeData.BeginTime = .TimeData.BeginTime.ToLocalTime
                        End If
                    Catch ex As Exception
                    ...
                    Try
                        ...
                        Else
                            .TimeData.EndTime = DateTime.UtcNow
                            .TimeData.EndTime = #1/1/2000#.AddSeconds(dEnd)
                            .TimeData.EndTime = .TimeData.EndTime.ToLocalTime
                        End If
                    Catch ex As Exception
                    ...
                    End Try
                    Try
                        .Project.Project = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Run number 
                    ReDim fBuff(0 To 1)
                    Try
                    ...
                    End Try
                    Try
                        .Project.Run = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Clone number 
                    ReDim fBuff(0 To 1)
                    Try
                    ...
                    End Try
                    Try
                        .Project.Clone = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Gen number 
                    ReDim fBuff(0 To 1)
                    Try
                    ...
                    End Try
                    Try
                        .Project.Gen = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Issued
                    ReDim fBuff(0 To 7)
                    Try
                    ...
                    Try
                        ...
                        Else
                            .Issued = DateTime.UtcNow    'try this to force the kind property to utc?
                            .Issued = #1/1/1970#.AddSeconds(dIssued) '
                            .Issued = .Issued.ToLocalTime
                        End If
                    Catch ex As Exception : End Try
                    'padding   36 bytes
                    ReDim fBuff(0 To 35)
                    ...
                    End Try
                    Try
                        .MachineID = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'ServerIP 4 bytes
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    End Try
                    Try
                        .ServerPort = BitConverter.ToInt16(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Wu type / message!
                    ReDim fBuff(0 To 63)
                    Try
                    ...
                    End Try
                    Try
                        .BenchMark = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Benchmark 2
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    End Try
                    Try
                        .CpuType = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'OsType
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    End Try
                    Try
                        .OsType = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Cpu species
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    Try
                        Array.Reverse(fBuff)
                        .Flops = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Memory 4 bytes
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    End Try
                    Try
                        .CpuMemory = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'gpu memory 4 bytes
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    End Try
                    Try
                        .GpuMemory = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'z624 64 bytes
                    ReDim fBuff(0 To 63)
                    Try
                    ...
                    End Try
                    Try
                        .PacketSizeLimit = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
                    'Failed uploads
                    ReDim fBuff(0 To 3)
                    Try
                    ...
                    Try
                        .UploadFailures = BitConverter.ToInt32(fBuff, 0)
                    Catch ex As Exception : End Try
            ...
            End Try
            Try
                _PerformanceUnits = BitConverter.ToInt32(fBuff, 0)
            Catch ex As Exception : End Try
            'Download rate
            ReDim fBuff(0 To 3)
            Try
            ...
            Try
                'Reverse fbuff
                'Array.Reverse(fBuff)
                _DownloadRate = (BitConverter.ToUInt32(fBuff, 0) / 1000.0F)
            Catch ex As Exception : End Try
            'Download rate units
            ReDim fBuff(0 To 3)
            Try
            ...
            End Try
            Try
                _DownloadRateUnits = BitConverter.ToInt32(fBuff, 0)
            Catch ex As Exception : End Try
            'Upload rate
            ReDim fBuff(0 To 3)
            Try
            ...
            Try
                'Reverse fbuff
                'Array.Reverse(fBuff)
                _UploadRate = (BitConverter.ToUInt32(fBuff, 0) / 1000.0F)
            Catch ex As Exception : End Try
            'Upload rate units
            ReDim fBuff(0 To 3)
            Try
            ...
            End Try
            Try
                _UploadRateUnits = BitConverter.ToInt32(fBuff, 0)
            Catch ex As Exception : End Try
            ...
            Return True
        Catch ex As Exception
            LogWindow.WriteError("clsQueue, ReadQueue", Err)
            _Err.Description = Err.Description
            _Err.Number = Err.Number
            _HasError = True
            _FullSlots = False
        ...
        End Try
NoGo:
        LogWindow.WriteError("clsQueue, ReadQueue", Err, "Condition NoGo triggerd, " & _Queue & " could not be succesfully read.")
        _Err.Description = Err.Description
        _Err.Number = Err.Number
        _HasError = True
        _FullSlots = False
...
#Region "Entry point"
    Sub New(ByVal Location As String)
        Try
            ...
            End If
        Catch ex As Exception
            LogWindow.WriteError("clsQueue, New()", Err)
            _HasError = True
            _Err.Description = Err.Description
            _Err.Number = Err.Number
        End Try
