...
Partial Public Class Formatting
    Private Shared ReadOnly log As ILog = LogManager.GetLogger(GetType(FAHWatch7.Business.Formatting))
    ...
    Public Shared ReadOnly Property FormatPPD(ByVal PPD As String) As String
        Get
            ...
            Catch ex As Exception
                log.ErrorFormat("Exception while formatting ppd from " & PPD & ": {0}", ex.Message)
                Return "-1"
            End Try
        End Get
    End Property
...
#Region "Timespan formatting"
    Public Shared Function Format_ts(ts As TimeSpan, Optional ms As Boolean = False) As String
        Try
            ...
            Return sb.ToString
        Catch ex As Exception
            log.ErrorFormat("Exception while formating a timespan (" & ts.ToString & "): {0}", ex.Message)
            Return "-1"
        End Try
    End Function
#End Region
